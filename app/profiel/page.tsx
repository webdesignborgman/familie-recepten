...
export default function ProfilePage() {
  ...
  useEffect(() => {
    if (!user) return;
    const q = query(collection(db, 'groepen'), where('leden', 'array-contains', user.uid));
    getDocs(q).then(snapshot => {
      setGroepen(
        snapshot.docs.map(doc => ({
          id: doc.id,
          ...(doc.data() as Omit<Groep, 'id'>),
        }))
      );
    });
  }, [user]);
        ...
        <div className="relative mb-2">
          {user.photoURL ? (
            <Image
              ...
              width={96}
              height={96}
              className="rounded-full border-4 border-[hsl(142,76%,36%)] bg-white object-cover shadow-md"
              priority
            />
          ) : (
        ...
        {groepen.map(g => (
          ...
          >
            ...
            <Button
              size="sm"
              variant="secondary"
              onClick={() => router.push(`/groepen/${g.id}`)}
              className="ml-4"
            >
              Beheren
            </Button>
